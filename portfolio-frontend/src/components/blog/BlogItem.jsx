...
const BlogItem = ({ isRu, strapiId, title, title_eng, image, date, category, slug,
    ...
    <li key={strapiId} className="blog">
      <figure className="blog__item">
        ...
        <figcaption className="blog__card">
          ...
          <p className="blog__card-text">{isRu ? description : description_eng}</p>
          ...
          <AniLink swipe direction="left"
                   to={`/blog/${slug}`}
                   className="blog__card-btn">{isRu ? 'Читать подробнее' : 'Read more'}</AniLink>
        </figcaption>
      </figure>
    </li>
  )
