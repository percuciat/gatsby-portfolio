...
const BlogList = ({isRu, blogItem, showLink }) => {
  ...
  const {allStrapiBlogsCategories: {nodes}} = useStaticQuery(graphql`
  ...
  const {actionFilter, filteredData, activeCategoryName} = useFilterCategory(blogItem);
        ...
        <ul className="blog__wrapper">
          {
            filteredData.length > 0 ?
              filteredData.map(blog => {
                return <BlogItem key={blog.strapiId}
                                 isRu={isRu}
                                 {...blog} />
              })
