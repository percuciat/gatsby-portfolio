...
const Project = ({ isRu, title, title_eng, image, slug,
                   shortDescription, shortDescription_eng }) => {
  return <>
      <figure className="projects__item">
        ...
        <figcaption className="projects__info">
          ...
          <p className="projects__description">{isRu ? shortDescription: shortDescription_eng}</p>
          <AniLink swipe direction="right"
                   to={`/projects/${slug}`}
                   className="projects__link">
            {isRu? 'Узнать подробнее': 'Read more'}</AniLink>
        </figcaption>
      </figure>
    </>
};
